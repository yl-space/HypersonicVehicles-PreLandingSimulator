...
export class CameraController {
    ...
    init() {
        ...
        // Start near the target once it's available; fallback is minimal offset
        this.camera.position.set(0, this.state.height, this.state.distance * 2);
        this.camera.lookAt(0, 0, 0);
        this.camera.up.set(0, 1, 0);  // Maintain standard Y-up orientation

        // Cinematic camera tracking state
        this.cinematic = {
            // Offset multipliers (scaled by state.distance)
            // x: right, y: up, z: back (negative forward)
            offset: new THREE.Vector3(0.0, 0.25, 1.1),
            smoothing: 0.08,  // Smooth position tracking
            orientationSmoothing: 0.05,  // Smoother orientation tracking
            lookAheadDistance: 0.00001,  // Look slightly ahead of spacecraft
    ...
    setupEventListeners() {
        ...
        canvas.addEventListener('wheel', (e) => {
            // Disable wheel zoom to avoid accidental zoom-outs
            e.preventDefault();
        }, { passive: false });
    ...
    /**
     * Snap camera immediately to a spacecraft-centric follow position.
     */
    ...
    update(deltaTime, vehicleData) {
        ...
        switch (this.mode) {
            ...
            case 'orbit':
                ...
                const minOrbitDistance = orbitMarsRadius + 0.00001;
                ...
                const minCraftDistance = this.state.minDistance;
                if (distToSpacecraft < minCraftDistance) {  // Maintain a few meters clearance
            ...
            case 'trajectory':
                // STATIONARY camera showing entire trajectory + live spacecraft.
                // Position computed once in computeTrajectoryView(), then held fixed.
                // The spacecraft moves along the trajectory; camera does NOT follow.
                if (this.trajectoryView.computed && this.trajectoryView.position) {
                    ...
                    // Fallback before trajectory is computed
                    desiredPosition.set(targetPos.x + 0.5, targetPos.y + 0.3, targetPos.z);
        ...
        if (this.mode === 'trajectory') {
            // Tight FOV to frame just the trajectory region
            if (Math.abs(this.camera.fov - 45) > 0.5) {
                this.camera.fov = THREE.MathUtils.lerp(this.camera.fov, 45, 0.1);
                this.camera.updateProjectionMatrix();
            }
        } else if (this.mode === 'follow' && vehicleData) {
            // NASA-style auto-zoom: wider FOV at high altitude (planet context),
            // slightly tighter near surface (terrain detail). Spacecraft should
            // occupy ~20-30% of frame width like NASA Eyes Mars 2020.
            const altitude = vehicleData.altitude || 100;
            const altNorm = THREE.MathUtils.clamp(altitude / 130, 0, 1);
            const targetFOV = THREE.MathUtils.lerp(50, 55, altNorm); // 55° high, 50° low
     ...
     */
    /**
     * Compute a STATIONARY camera that tightly frames the trajectory arc.
     * The camera should show ONLY the trajectory region (not the whole planet).
     * The spacecraft moves along the trajectory while the camera stays fixed.
     */
    computeTrajectoryView(trajectoryPoints) {
        ...
        // Direction along the trajectory arc
        const along = end.clone().sub(start);
        if (along.length() < 0.0001) along.copy(midPoint).sub(start);
        ...
        if (side.length() < 0.001) side.set(1, 0, 0);
        side.normalize();

        // Max altitude above surface
        const planetRadius = this.orbit.planetRadius;
        let maxAlt = 0;
        ...
        // Frame ONLY the trajectory region — arc length × altitude extent.
        // Use a tight FOV (45°) and place camera just far enough to fit.
        const extent = Math.max(arcLength, maxAlt, 0.005);
        const fovRad = 45 * Math.PI / 180;
        const camDist = (extent * 0.5) / Math.tan(fovRad / 2) * 1.2; // 1.2x padding

        // Look at the midpoint of the trajectory, elevated by half the max altitude
        const lookAt = midPoint.clone().add(radial.clone().multiplyScalar(maxAlt * 0.3));

        // Camera: offset to the side, slightly above the trajectory plane
        const camPos = lookAt.clone();
        camPos.add(side.clone().multiplyScalar(camDist));
        camPos.add(radial.clone().multiplyScalar(maxAlt * 0.2));

        // Ensure camera is above planet surface
        const camDistFromCenter = camPos.length();
        if (camDistFromCenter < planetRadius + maxAlt * 0.5) {
            const pushDir = camPos.clone().normalize();
            camPos.copy(pushDir.multiplyScalar(planetRadius + maxAlt * 0.5));
        }
    ...
    reset() {
        ...
        if (this.target) {
            ...
            this.camera.position.set(0, 0.0001, this.state.defaultDistance * 2);
            this.camera.lookAt(0, 0, 0);
        ...
        this.orbit.radius = this.state.defaultDistance * 1.5;
